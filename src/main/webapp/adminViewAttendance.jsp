...
<head>
    ...
    <style>
        :root { --primary: #0056b3; --danger: #dc2626; --success: #10b981; --warning: #f59e0b; }
        body { background-color: #f8fafc; font-family: 'Segoe UI', Tahoma, sans-serif; }
        
        .filter-card { background: white; padding: 25px; border-radius: 15px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); margin-bottom: 25px; }
        .form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; align-items: end; }
        
        .summary-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 25px; }
        .stat-card { background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.03); text-align: center; border-bottom: 5px solid var(--primary); }
        .stat-card.red { border-bottom-color: var(--danger); }
        .stat-card h3 { margin: 0; font-size: 2rem; color: #1e293b; }
        .stat-card p { margin: 5px 0 0; color: #64748b; font-weight: 600; text-transform: uppercase; font-size: 0.8rem; }

        .report-wrapper { background: white; border-radius: 15px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.05); }
        .data-table { width: 100%; border-collapse: collapse; }
        .data-table th { background: var(--primary); color: white; padding: 15px; font-size: 0.85rem; text-transform: uppercase; }
        .data-table td { padding: 12px 15px; border-bottom: 1px solid #f1f5f9; text-align: center; }
        
        .pct-badge { padding: 5px 12px; border-radius: 20px; font-weight: 700; font-size: 0.85rem; }
        .bg-low { background: #fee2e2; color: var(--danger); }
        .bg-good { background: #dcfce7; color: #166534; }
        
        .btn-bulk { background: #8b5cf6; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.3s; }
        .btn-bulk:hover { background: #7c3aed; transform: translateY(-2px); }
        
        .select-field, .date-field { width: 100%; padding: 10px; border: 1px solid #e2e8f0; border-radius: 8px; margin-top: 5px; }
    </style>
</head>
<body>
    ...
    <div class="dashboard-wrapper" style="padding: 20px; max-width: 1400px; margin: auto;">
        
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h2 style="color: #1e293b; margin: 0;"><i class="fas fa-chart-line"></i> Attendance Analytics</h2>
        </div>
        ...
        <div class="filter-card">
            <form method="GET" class="form-grid">
                <div>
                    ...
                    <select name="class" class="select-field" required>
                        <option value="" disabled selected>-- Select Type --</option>
                        <option value="MSC-CS" <%= "MSC-CS".equals(request.getParameter("class")) ? "selected" : "" %>>MSc Computer Science</option>
                        <option value="MCA" <%= "MCA".equals(request.getParameter("class")) ? "selected" : "" %>>MCA</option>
                        <option value="BTECH-IT" <%= "BTECH-IT".equals(request.getParameter("class")) ? "selected" : "" %>>B.Tech IT</option>
                    </select>
                </div>
                <div>
                    <label>Join Year</label>
                    <select name="joinyear" class="select-field" required>
                ...
                <div>
                    ...
                    <input type="date" name="from" class="date-field" value="<%= request.getParameter("from") != null ? request.getParameter("from") : "" %>" required>
                </div>
                <div>
                    <label>End Date</label>
                    <input type="date" id="endDate" name="to" class="date-field" value="<%= request.getParameter("to") != null ? request.getParameter("to") : "" %>" required>
                </div>
                <button type="submit" class="save-btn" style="background: var(--primary); height: 45px; border-radius: 8px;">
                    ...
                    <tbody>
                        <%
                            try (Connection con = DbConnection.getConne()) {
                                // SQL using JOIN to handle missing class_name in student_attendance
                                ...
                                // Ambiguity Fix: Explicitly use java.sql.Date
                        ...
                        <tr>
                            ...
                            <td><%= attendedCount %></td>
                            <td>
                                <span class="pct-badge <%= (pct < 75 ? "bg-low" : "bg-good") %>" 
                                      data-pct="<%= String.format("%.2f", pct) %>" 
                                      data-status="<%= (pct < 75 ? "low" : "good") %>">
                                    <%= String.format("%.2f", pct) %>%
                                </span>
                            </td>
                            <td>
                                <button onclick="sendSingleAlert('<%= rs.getString("email") %>', '<%= String.format("%.2f", pct) %>')" 
