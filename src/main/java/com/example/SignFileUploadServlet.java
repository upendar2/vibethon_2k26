...
@WebServlet("/uploadsign")
@MultipartConfig
public class SignFileUploadServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (filePart != null && filePart.getSize() > 0 && regdNo != null && !regdNo.isEmpty()) {
                ...
                try (InputStream inputStream = filePart.getInputStream();
                    ...
                    String sql = "UPDATE students SET sign = ? WHERE regd_no = ?";
                    try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                        
                        // CHANGE 1: Use setBinaryStream for PostgreSQL's BYTEA data type
                        ...
                        if (rowsAffected > 0) {
                            // Fetch the next student who needs a signature
                            // CHANGE 2: Use length() instead of dbms_lob.getlength()
                            String nextStudentSql = "SELECT regd_no, name FROM students WHERE (sign IS NULL OR length(sign)=0) AND regd_no > ? ORDER BY regd_no ASC";
                            ...
                            isSuccess = true;
                            message = "Successfully uploaded signature for " + submittedStudentName;
                        } else {
                            conn.rollback();
        ...
        StringBuilder redirectURL = new StringBuilder("signFileUpload.jsp");
